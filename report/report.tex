...
\title{Velocity Anisotropy of Satellites in IllustrisTNG-100}
...
\begin{center}
\href{https://github.com/pikaism/TNG-anisotropy-atlas}{github.com/pikaism/TNG-anisotropy-atlas}
\end{center}

\begin{abstract}
We present a reproducible pipeline for measuring satellite galaxy
velocity anisotropy $\beta(r)$ in IllustrisTNG-100 (snapshot 99,
$z=0$), validated on synthetic halos with known input $\beta$ before
application to real data. Using bootstrap errors that resample host
halos rather than individual satellites, we measure $\beta(r)$ across
five host-halo mass bins, finding a monotonic trend from
tangentially-biased orbits at group scale to radially-biased orbits at
cluster scale, consistent with observational and N-body expectations.
We further examine environment, star-forming/quenched status, and a
resolved-galaxy stellar-mass cut, and perform two resolution checks
(halo mass function, stellar-mass floor) confirming our sample is
unaffected by simulation resolution limits. This is an exploratory,
validated pilot analysis, intended as a baseline for future
population-level and cross-simulation comparisons.
\end{abstract}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth]{../figures/halo_mass_function.png}
\caption{Halo mass function for TNG100-1 snapshot 99. The dashed line
marks the $\sim$50-particle resolution limit; the shaded region shows
the mass range used in this analysis, which lies well above it.}
\label{fig:massfunc}
\end{figure}
...
halos with known input $\beta$. Across 30 random realizations each for
$\beta_\mathrm{input} \in \{-0.5, 0, 0.5\}$, recovered values matched
inputs to within $\sim$1$\sigma$. A radius-dependent test case,
$\beta(r) = 0.5(r/r_\mathrm{max})^2$, was also correctly recovered in
shape. (See \texttt{notebooks/01\_toy\_validation.ipynb} for the full
validation figures.)
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{../figures/quenched_fraction_radial.png}
\caption{Quenched fraction vs.\ clustercentric radius for the
galaxy-resolved Bin 3 and Bin 4 samples, with bootstrap errors
resampling host halos.}
\label{fig:quenchradial}
\end{figure}
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{../figures/beta_vs_mass.png}
\caption{Stacked $\beta(r)$ profiles across the five host-halo mass
bins, with bootstrap errors resampling host halos.}
\label{fig:betamass}
\end{figure}
...
($p = 1.55\times10^{-12}$). Controlling for mass via partial Spearman
correlation:
\begin{equation}
\rho(\beta, \mathrm{environment} \mid \mathrm{mass}) = -0.099,
\qquad p = 4.19\times10^{-7},
\end{equation}
indicating environment has a statistically significant effect on
$\beta$ independent of mass.

A median environment split within each mass bin shows a sign reversal in
$\Delta\beta$ in
$\Delta\beta$
(Figure~\ref{fig:envsplit}): at group/intermediate mass, denser
environments correspond to \emph{more} tangential bias ($\Delta\beta$
from $-0.073$ to $-0.084$ across Bins 0--3), while at cluster mass,
denser environments correspond to \emph{more} radial bias
($\Delta\beta = +0.048$ in Bin 4). We interpret this as increased
tidal stirring at group scale versus enhanced filamentary infall
feeding actively-growing clusters, but note Bin 4's environment split
has only 62--65 halos per half; we present this as a suggestive,
physically-motivated finding warranting further investigation with
larger-volume simulations, not a confirmed result.

\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{../figures/beta_environment_split.png}
\caption{Median environment split of $\beta$ within each mass bin.
Note the sign reversal in the cluster-mass bin.}
\label{fig:envsplit}
\end{figure}
...
radially biased than the full subhalo population
(Figure~\ref{fig:galfull}; e.g.\ Bin 3: $-0.019 \to +0.144$; Bin 4:
$+0.162 \to +0.278$), consistent with stronger dynamical friction
acting on more massive, luminous satellites.

\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{../figures/beta_galaxy_vs_full.png}
\caption{Comparison of $\beta$ from the full subhalo population versus
the resolved-galaxy-only subsample, Bins 2--4.}
\label{fig:galfull}
\end{figure}

Restricting to the same resolved-galaxy bins, satellites also show a
clear split by star-formation status (Figure~\ref{fig:sfq}): quenched
satellites are consistently more radially biased than star-forming
ones, consistent with satellites on radial, eccentric orbits plunging
closer to the host center, where ram-pressure stripping and tidal
quenching are strongest. Bin 2 has zero quenched-hosting halos, a
physically sensible null result given group-mass halos generally lack
the hot gas atmosphere needed for ram-pressure quenching.

\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{../figures/beta_sf_quenched.png}
\caption{$\beta$ split by star-forming vs.\ quenched satellite status,
Bins 3--4.}
\label{fig:sfq}
\end{figure}
